...
export function createServer(): McpServer {
  ...
  server.tool(
    'list_components',
    'List all SolarUI components grouped by category (Action, Form, Feedback, Layout, Navigation, Data, Chart, Core). Use this to discover what components are available.',
    {},
    async () => {
  ...
  server.tool(
    'find_component',
    'Find the right SolarUI component for a given use case. Describe what you need in plain English and get back a ranked list of matching components.',
    { query: z.string().describe('Plain-English description, e.g. "show confirmation before delete", "display user status inline", "navigate between sections"') },
    ({ query }) => {
  ...
  server.tool(
    'get_color_token',
    'Look up SolarUI color tokens. Returns the semantic meaning of each step (1–12) and which Tailwind classes to use. Also includes critical anti-patterns (no dark: prefixes, no shadcn default tokens).',
    {
  ...
  server.tool(
    'get_guidelines',
    'Get SolarUI design guidelines and critical rules. Includes color token usage, dark mode approach, control height system, and common anti-patterns AI agents must avoid.',
    {
  ...
  server.resource(
    'component-registry',
    'solar://components',
    { mimeType: 'application/json', description: 'Complete SolarUI component registry with categories, variants, and descriptions. Load this at the start of a UI-building session.' },
    async () => {
  ...
  server.resource(
    'color-system',
    'solar://colors',
    { mimeType: 'application/json', description: 'Complete SolarUI color token system: 6 semantic groups × 12 steps = 72 Tailwind utilities with semantic roles and anti-patterns.' },
    () => {
  ...
  server.resource(
    'design-guidelines',
    'solar://guidelines',
    { mimeType: 'text/markdown', description: 'SolarUI design principles, token usage rules, and critical anti-patterns (dark mode, shadcn token names, etc.).' },
    () => {
  ...
  server.resource(
    ...
    { mimeType: 'text/markdown', description: 'Full documentation for a specific SolarUI component. URI format: solar://components/button' },
    async (uri, { name }) => {
